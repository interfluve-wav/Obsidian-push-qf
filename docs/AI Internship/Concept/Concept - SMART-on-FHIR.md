...
#capstone #ai-internship #neurology #concept #interoperability-standard
...
## SMART-on-FHIR in the Abrige Workflow

From the KUMC study and Abrige's documentation:

```
1. Clinician opens Abrige app
        ↓
2. Abrige reads patient schedule from Epic (via SMART-on-FHIR)
        ↓
3. Clinician records the patient encounter
        ↓
4. Abrige generates the clinical note
        ↓
5. Abrige writes the note back to Epic (via SMART-on-FHIR)
        ↓
6. Clinician uses dot-phrases (.hpisec, .meds, etc.) to pull
   sections into the Epic note template
```

This is the integration architecture behind Abrige's EHR write-back capability.
...
|---|---|---|
| **Abrige** | Deep — reads schedule, writes notes, supports dot-phrases | Considered deepest ambient AI integration |
...
- [[Concept - Epic]]
- [[Concept - Cerner and athenahealth]]
- [[Concept - Dot-Phrase]]
